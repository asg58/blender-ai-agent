...
async def run_in_blender(code):
    ...
    try:
        ...
        return {
            "error": f"Connection closed: The connection to Blender was closed",
            "details": str(e)
        }
    except ConnectionRefusedError as e:
        return {
            "error": f"Connection refused: Could not connect to Blender at {BLENDER_WS_URL}. Is Blender running with the WebSocket server enabled?",
            "details": str(e)
        }
    except socket.gaierror as e:
        return {
            "error": f"Network error: Could not resolve host in {BLENDER_WS_URL}",
            "details": str(e)
        }
